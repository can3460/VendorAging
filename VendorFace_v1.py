...
if uploaded_file:
    if st.button("🚀 Run Audit Analysis", type="primary", use_container_width=True):
        with st.spinner("Processing..."):
            ...
            st.session_state['audit_results'] = {'gl_final': gl_final, 'v_ap': v_ap, 'v_db': v_db, 'rec_df': pd.DataFrame(rec_list) if rec_list else None, 'gap_df': pd.DataFrame(gap_list) if gap_list else None, 'buckets': buckets}
...
if st.session_state['audit_results']:
    ...
    gl_disp = scale_it(res['gl_final'], res['buckets'] + ['Total Balance'])
    ap_disp = scale_it(res['v_ap'], res['buckets'] + ['Total Balance'])
    db_disp = scale_it(res['v_db'], res['buckets'] + ['Total Balance'])
    rec_disp = scale_it(res['rec_df'], ['TB_Balance', 'FBL1n_Sum', 'Difference'])
    gap_disp = scale_it(res['gap_df'], ['TB Balance'])
    ...
    with col_e2:
        ...
        st.download_button("📥 Export Dashboard to Excel", output.getvalue(), f"Audit_{display_unit}.xlsx", use_container_width=True)
