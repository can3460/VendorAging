...
# Analiz sonuçlarını saklamak için session_state
...
def smart_parse_tb(file):
    ...
    try:
        ...
        # Sütun Yakalama (Account Number Odaklı)
...
if st.session_state['audit_results']:
    ...
    gl_disp = scale_df(res['gl_final'], res['buckets'] + ['Total Balance'])
    ap_disp = scale_df(res['v_ap'], res['buckets'] + ['Total Balance'])
    db_disp = scale_it = scale_df(res['v_db'], res['buckets'] + ['Total Balance'])
    rec_disp = scale_df(res['rec_df'], ['TB_Balance', 'FBL1n_Sum', 'Difference']) if res['rec_df'] is not None else None
    gap_disp = scale_df(res['gap_df'], ['TB Balance']) if res['gap_df'] is not None else pd.DataFrame()
    ...
    with col_e1:
        ...
        st.download_button("📄 Print Dashboard (HTML Export)", html_out, f"Audit_Dashboard_{display_unit}.html", "text/html", use_container_width=True)
    ...
    with col_e2:
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            ...
            if rec_disp is not None: rec_disp.to_excel(writer, sheet_name='Reconciliation', index=False)
            if not gap_disp.empty: gap_disp.to_excel(writer, sheet_name='TB Check', index=False)
        st.download_button("📥 Export Dashboard to Excel", output.getvalue(), f"Audit_Pack_{display_unit}.xlsx", use_container_width=True)
    ...
    st.info("👋 Welcome! Please upload your reports and click 'Run Audit Analysis'.")
