...
# --- USER DATABASE FUNCTIONS (HOTFIXED) ---
def load_users():
    ...
    # Sütun isimlerini küçük harfe çevirerek KeyError'ı engelle
...
# Dashboard UI & Logic
st.markdown(f"### 📊 AP Analyzing Suite - {display_unit if 'results' in st.session_state else ''}")
...
if st.session_state['results']:
    ...
    with pd.ExcelWriter(output_full, engine='xlsxwriter') as writer:
        ...
        res['raw_data'].head(5000).to_excel(writer, sheet_name='Raw Data', index=False)
    st.download_button("📥 Download Detailed Audit Report (Full Data Pack)", output_full.getvalue(), f"Audit_Pack_{datetime.now().strftime('%Y%m%d')}.xlsx", use_container_width=True, type="primary")
